...
var ViewabilityTracker = /** @class */ (function () {
    ...
    ViewabilityTracker.prototype._itemIntersectsVisibleWindow = function (startBound, endBound) {
        ...
        var visibleHeight = this._visibleWindow.end - this._visibleWindow.start;
        var itemHeight = endBound - startBound;
        var needsToCover = 60;
        var mode = "view";
        ...
        if ((startBound > this._visibleWindow.start && endBound < this._visibleWindow.end)
            ...
            var visibleItemHeight = this._visibleWindow.end - Math.max(this._visibleWindow.start, startBound);
            var visiblePixels = Math.max(0, visibleItemHeight);
            var percentage = 100 * (visiblePixels / (mode === "view" ? visibleHeight : itemHeight));
            ...
            var visibleItemHeight = Math.min(this._visibleWindow.end, endBound) - Math.max(this._visibleWindow.start, 0);
            var visiblePixels = Math.max(0, visibleItemHeight);
            var percentage = 100 * (visiblePixels / (mode === "view" ? visibleHeight : itemHeight));
            return percentage >= needsToCover;
        }
